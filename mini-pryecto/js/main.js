...
function runProgram(button) {
  ...
  percentageButtons.forEach((boton) => {
    boton.classList.remove("active");
  });

  button.addEventListener("click", claseActivo(button));
  ...
  if (bill > 0 && tip > 0 && people > 0) {
    ...
    billInput.classList.remove("red");
    peopleInput.classList.remove("red");
    customPercentageBtn.classList.remove("red");

    resetBtn.style.display = "block";

  } else if (esCero(bill) == true) {
    billInput.classList.add("red");
  } else if (esCero(people) == true) {
    peopleInput.classList.add("red");
  } else if (esCero(tip) == true) {
    percentageButtons.classList.add("red");
  }
}
